...
e1000_recv(void)
{
  ...
  while(1) {
    ...
    if(rx_ring[next].status & E1000_RXD_STAT_DD) {
      struct mbuf* nmbufptr = mbufalloc(0);

      if(nmbufptr) {
        mbufptr = rx_mbufs[next];
        mbufptr->len = rx_ring[next].length;

        rx_mbufs[next] = nmbufptr;
        rx_ring[next].addr = (uint64)nmbufptr->head;
      }
